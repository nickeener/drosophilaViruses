#!/usr/bin/python3
# FROM: https://github.com/pirovc/metameta/blob/master/scripts/randomReadSubSample.py
...
'''
usage: randomReadSubSample.py [-h] [-f <fasta_q_files>]
                              [-f1 <paired1_fasta_q_files>]
                              [-f2 <paired2_fasta_q_files>] [-s <sample_size>]
                              [-n <sample_number>] [-r <replacement>]
                              [-g <gzip_output>] [-o <output_prefix>]
 
optional arguments:
  -h, --help            show this help message and exit
  -f <fasta_q_files>    fasta/fastq [.gz] single file
  -f1 <paired1_fasta_q_files>
                        first paired fasta/fastq [.gz] file
  -f2 <paired2_fasta_q_files>
                        second paired fasta/fastq[.gz] file
  -s <sample_size>, --sample-size <sample_size>
                        Total amount of reads per sample. Integer value to set
                        a specific read number (e.g. 100 -> 100 reads), float
                        value for percentage (e.g. 0.1 -> 10% of the reads), 1
                        to use the whole set (e.g. 1 -> all reads). Default:
                        0.1
  -n <sample_number>, --sample-number <sample_number>
                        Number of samples. Default: 1
  -r <replacement>, --replacement <replacement>
                        Sampling with replacement [1] or without replacement
                        [0]. Default: 0
  -g <gzip_output>, --gzip-output <gzip_output>
                        Output flat [0] or gziped [1] files. Default: 0
